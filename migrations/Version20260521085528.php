...
final class Version20260521085528 extends AbstractMigration
{
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('SET @sql := IF((SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = "stock") = 1 AND (SELECT COUNT(*) FROM information_schema.table_constraints WHERE table_schema = DATABASE() AND table_name = "stock" AND constraint_name = "FK_4B3656604584665A") = 1, "ALTER TABLE stock DROP FOREIGN KEY FK_4B3656604584665A", "SELECT \'No-op: stock FK missing\'");');
        $this->addSql('PREPARE stmt FROM @sql;');
        $this->addSql('EXECUTE stmt;');
        $this->addSql('DEALLOCATE PREPARE stmt;');
        $this->addSql('SET @sql := IF((SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = "stock") = 1 AND (SELECT COUNT(*) FROM information_schema.table_constraints WHERE table_schema = DATABASE() AND table_name = "stock" AND constraint_name = "FK_4B365660B03A8386") = 1, "ALTER TABLE stock DROP FOREIGN KEY FK_4B365660B03A8386", "SELECT \'No-op: stock FK missing\'");');
        $this->addSql('PREPARE stmt FROM @sql;');
        $this->addSql('EXECUTE stmt;');
        $this->addSql('DEALLOCATE PREPARE stmt;');
        ...
        $this->addSql('EXECUTE stmt;');
        $this->addSql('DEALLOCATE PREPARE stmt;');
        $this->addSql('SET @sql := IF((SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = "products") = 1 AND (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = "products" AND column_name = "stock") = 0, "ALTER TABLE products ADD stock INT DEFAULT NULL", "SELECT \'No-op: products missing or column exists\'");');
        $this->addSql('PREPARE stmt FROM @sql;');
        $this->addSql('EXECUTE stmt;');
        $this->addSql('DEALLOCATE PREPARE stmt;');
    }
